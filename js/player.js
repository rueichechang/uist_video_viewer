...
class Player {
  ...
  _activate(baseIdx, startSeg = 0, startOffset = 0) {
    ...
    // Decide the starting segment + media time. A resume plan (first activation
    // only) wins; otherwise honor an explicit seek target (startSeg/startOffset
    // from a scrubber jump); otherwise start at the first segment's IN.
    let startAt = null;
    if (this._resumeSpot && this._resumeSpot.kind === 'video') {
      ...
      if (this._segIdx !== 0 || startOffset > 0.01) {
        const s = this._segs[this._segIdx];
        startAt = (s.inP || 0) + Math.max(0, startOffset);
      }
    }
    this._resumeSpot = null;
    const seg = this._segs[this._segIdx];
    ...
    this._updateProgress();
    // Normal entry plays from IN (already seeked during _prepare). A resume or
    // a scrubber jump sets startAt; seek there before playing/arming. Kept
    // synchronous so start()'s transition lock still covers the first activation.
